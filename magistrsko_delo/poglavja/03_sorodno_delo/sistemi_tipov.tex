...
Programski jezik Granule~\cite{orchard2019quantitative} je \textit{neučakan} \komentar{močno tipiziran} (angl. strongly typed) funkcijski jezik, ki v svojem sistemu tipov združuje linearne, indeksne in \komentar{graded modal} (angl. graded modal) tipe. Granule uporablja princip podatkov kot virov (angl. data as a resource) v svojem sistemu tipov. Za upravljanje s pomnilnikom je uporabljen avtomatičen čistilec. 
...
Na tem mestu je še vredno omeniti, da je programski jezik Granule \textit{neučakan}. \komentar{``Call-by-value was chosen for simplicity and since the effectful part of Gr is necessarily CBV (to avoid pitfalls of laziness with side effects).``~\cite{orchard2019quantitative}. Preveriti moram še, zakaj je nujno imeti prenos po vrednosti.}
