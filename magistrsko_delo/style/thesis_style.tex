...
\floatstyle{plain}
\newfloat{primer}{ht}{lop}[chapter]
\floatname{primer}{Primer}
\captionsetup[primer]{labelfont=bf}
...
\newtcblisting{code-box-breathe}[3][]{%
	listing engine=minted,
	minted options={
		linenos,
		breaklines,
...
\newcommand{\infers}[4][]{
	\frac{#2}{#3 \enspace \leadsto \enspace #4}
	\if\relax\detokenize{#1}\relax
	\notag
	\else
	\tag{\textsc{#1}}
...
\newcommand{\object}[1]{\textsc{\MakeLowercase{#1}}}